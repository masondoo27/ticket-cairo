...
pub trait ITicket<TContractState> {
    fn getCost(self: @TContractState, numberOfLotToBuy: u128) -> u128;
    ...
    fn updateTicketPrice(ref self: TContractState, newPrice: u128);
    ...
    fn getTicketPrice(self: @TContractState) -> u128;
...
mod TicketStarknet {
    ...
    use openzeppelin::utils::{
        selectors, try_selector_with_fallback
    };
    ...
    pub impl InternalImpl of InternalImplTrait {
        fn _process_payment(
            ...
            self._payment_transfer_from(payment_token, sender, receiver, amount);
        }
    ...
    impl TicketStarknetImpl of super::ITicket<ContractState> {
        fn getCost(self: @ContractState, numberOfLotToBuy: u128) -> u128{
            let cost:u128 = self.ticket_price.read() * numberOfLotToBuy;
            let mut discount:u128 = 0;
             if numberOfLotToBuy >= 5 {
                discount = (self.discount5.read() * cost) / 100;
            }
            if numberOfLotToBuy >= 10 {
                discount = (self.discount10.read() * cost) / 100;
            }
        ...
        fn buyTickets(ref self: ContractState, numberOfLotToBuy: u128){
            ...
            let cost: u256  = self.getCost(numberOfLotToBuy).into();
            ...
            self.emit(super::TicketsPurchased { 
                buyer,
        ...
        fn updateTicketPrice(ref self: ContractState, newPrice: u128){
            self.ownable.assert_only_owner();
        ...
        fn getTicketPrice(self: @ContractState) -> u128{
